    ...
    return new Promise(
      (resolve, reject) => {
        ...
        if (typeof callback !== 'function') {
          AWSService.debug(`Executing generic method '${method}' without callback!`)
        }
        ...
        async.doWhilst(
          ...
          (err) => {
            const response = this.reduceResponses(responses)
  ...
  exec () {
    ...
    return new Promise((resolve, reject) => {
      if (typeof this.handler[method] !== 'function') {
        reject(new Error(`Invalid service method: ${this.name}:${method}`))
      }
